...
let WebSearchService = class WebSearchService {
     ...
     */
    async fetchLiveRates(city, locality, quality) {
    ...
    async searchConstructionRates(location, quality) {
        const year = new Date().getFullYear();
        const query = `residential house construction cost per sq ft ${location} India ${year} ` +
...
`);
    ...
    // ── 2. Land / plot rates ───────────────────────────────────────────────────
    async searchLandRates(location) {
        const year = new Date().getFullYear();
        const query = `residential plot land price per square yard ${location} India ${year} ` +
...
`);
        if (!extracted.low || !extracted.high)
            return undefined;
    ...
    async searchMaterialPrices() {
        const year = new Date().getFullYear();
        const query = `India construction material prices ${year}: OPC cement 50kg bag price, ` +
...
`);
    ...
    async extractNumbers(sourceText, instructions) {
        const model = this.genAI.getGenerativeModel({
            model: 'gemini-2.0-flash',
...
`.trim();
    ...
    defaultConstructionRates(city, quality) {
    ...
    fallbackRates(city, quality, now) {
...
WebSearchService = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [])
], WebSearchService);
export { WebSearchService };
//# sourceMappingURL=costestimator.web-search.js.map