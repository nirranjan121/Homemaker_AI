...
var _a;
import { ToolDecorator as Tool, z, ControllerDecorator, } from '@nitrostack/core';
...
let CostEstimatorTools = class CostEstimatorTools {
    state;
    houseplanState;
    geocoder;
    webSearch;
    constructor(state, houseplanState, geocoder, webSearch) {
        this.state = state;
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async estimateFullCost(input, _ctx) {
        ...
        if (!houseAreaSqFt) {
            if (this.houseplanState.has()) {
                ...
                throw new Error('No floor area provided and no active house plan found. ' +
        ...
        if (!liveRates) {
            liveRates = await this.webSearch.fetchLiveRates(locationInfo.city, locationInfo.locality, input.quality);
        ...
        const materialBreakdown = estimateMaterials(totalAreaForConstruction, input.floors, input.quality, liveRates.materialPrices);
        ...
        const estimate = {
            ...
            totalProjectCost,
            rates: liveRates,
            disclaimer: 'Feasibility-stage estimate based on live internet data. ' +
        ...
        if (landCost) {
            summaryParts.push(`🌍 **Land Cost** (${input.plotAreaSqYd} sq yd): ${formatInr(landCost.low)} – ${formatInr(landCost.high)}`);
        }
        summaryParts.push(`🧱 **Material Cost Estimate**: ${formatInr(matTotal)}`, `💰 **Total Project Cost**: ${formatInr(totalProjectCost.low)} – ${formatInr(totalProjectCost.high)}`, `📊 **Live Rate Used**: ₹${cLow}–₹${cHigh}/sq ft (construction) — fetched ${new Date(liveRates.fetchedAt).toLocaleTimeString('en-IN')}`);
        if (liveRates.sources.length > 0) {
            summaryParts.push(`🔗 **Sources**: ${liveRates.sources.slice(0, 3).join(', ')}`);
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    async refineCostEstimate(input, ctx) {
        const existing = this.state.getEstimate();
        if (!existing) {
            throw new Error('No existing estimate to refine. Call estimate_full_cost first.');
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    getMaterialBreakdown(_input, _ctx) {
        const existing = this.state.getEstimate();
        if (!existing) {
            throw new Error('No estimate available. Call estimate_full_cost first.');
        }
        return {
...
__decorate([
    Tool({
        ...
        description: 'Compute a detailed, internet-sourced construction + land + material cost estimate. ' +
        ...
        inputSchema: z.object({
            location: z
                .string()
                .describe('Location for the house, e.g. "Whitefield, Bengaluru" or "Andheri West, Mumbai". ' +
                'Be as specific as possible for more accurate rates.'),
            quality: z
                .enum(['basic', 'standard', 'premium'])
                .default('standard')
                .describe('Construction quality tier: basic (economy finishes), ' +
            ...
            houseAreaSqFt: z
                ...
                .positive()
                .optional()
                .describe('Total built-up area in sq ft. If omitted, the area is read from the active ' +
                '3D floor plan model (generate_3d_shell must have been called first).'),
            plotAreaSqYd: z
                .number()
                .positive()
                .optional()
                .describe('Plot / land area in square yards. Provide this to include a land cost estimate. ' +
...
], CostEstimatorTools.prototype, "estimateFullCost", null);
__decorate([
    Tool({
        name: 'refine_cost_estimate',
        description: 'Adjust an existing cost estimate by changing one or more parameters ' +
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], CostEstimatorTools.prototype, "refineCostEstimate", null);
__decorate([
    Tool({
        name: 'get_material_breakdown',
        description: 'Return the detailed itemised material breakdown from the most recent cost estimate. ' +
...
], CostEstimatorTools.prototype, "getMaterialBreakdown", null);
CostEstimatorTools = __decorate([
    ControllerDecorator(),
    __metadata("design:paramtypes", [CostEstimatorState, typeof (_a = typeof HouseplanState !== "undefined" && HouseplanState) === "function" ? _a : Object, GeocoderService,
        WebSearchService])
], CostEstimatorTools);
export { CostEstimatorTools };
//# sourceMappingURL=costestimator.tools.js.map